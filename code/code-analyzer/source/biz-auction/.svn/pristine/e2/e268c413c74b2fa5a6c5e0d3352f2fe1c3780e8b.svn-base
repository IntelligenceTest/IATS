...
<div class="main clearfix">
	...
	<div class="left">
		...
		<div class="left-selected clearfix" style='display:block;'>
			...
			<ul id="carSelected">
				...
				<#if (price?? && price > 0)>
					<li id="carPrice1">${action.paramName(1,price)}<a href="${action.getUrl("price,min,max,pageNumber", "0,,,1")}">X</a></li>
				<#else>
		...
		<div class="left-option">
			<div class="content newcarlist_brand">
				...
				<ul class="clearfix" id="carBrand">
					...
					<li>
						<a href="${action.getUrl("brand,pageNumber", "${param.name},1")}" <#if brand?? && brand?string == param.name> class="selected"</#if> onClick="_traceEvent('搜索按钮', '点击', '${(param.name)!},${(city)!},${(price)!},${(mile)!},${(age)!}','2','false');">
			...
			<div class="content clearfix">
				...
				<ul id="carPrice">
                    <#list action.paramList(1) as param>
						<li>
                      		<a href="${action.getUrl("price,min,max,pageNumber", "${param.key},,,1")}" <#if price?? && price?string == param.key> class="selected"</#if> onClick="_traceEvent('搜索按钮', '点击', '${(brand)!},${(city)!},${(param.key)!},${(mile)!},${(age)!}','2','false');">${param.value}</a>
                      	</li>
                    </#list>
				</ul>
			...
			<div class="content clearfix" id="Div2">
				...
				<ul id="carMile">
					<#list action.paramList(3) as param>
                    	<li>
							<a href="${action.getUrl("mile,pageNumber", "${param.key},1")}" <#if mile?? && mile?string == param.key> class="selected"</#if> onClick="_traceEvent('搜索按钮', '点击', '${(brand)!},${(city)!},${(price)!},${(param.key)!},${(age)!}','2','false');">${param.value}</a>
						</li>
                    </#list>
				</ul>
			</div>
			...
			<div class="content clearfix" id="Div3">
				...
				<#list action.paramList(2) as param>
					<li>
						<a href="${action.getUrl("age,pageNumber", "${param.key},1")}" <#if age?? && age?string == param.key> class="selected"</#if> onClick="_traceEvent('搜索按钮', '点击', '${(brand)!},${(city)!},${(price)!},${(mile)!},${(param.key)!}','2','false');">${param.value}</a>
					</li>
				</#list>
				</ul>
			</div>
            ...
            <#if topAdDetails??&&topAdDetails?size gt 0>
            	<ul class="adv_list">
            		<#list topAdDetails as detail>
	               	 	<li><a href="${(detail.clickUrl)!"javascript:;"}" target="_blank"><img src="<@baseUrl.image/>/${action.getAdPic('${(detail.adDesign)!}','190*114')!}"></a></li>
	                </#list>
            	</ul>
            </#if>
	...
	<div class="right">
				...
				<div id="newCityList" class="newcarlist_top_city">
					<div class="newcity-top">
						<ul id="TopUl">
			                ...
			                <li id="TopUlC"><span><#if city?? && province != city>${(city)!}</#if></span><img src="<@baseUrl.static />/new_shop/build/2.00/images/toDown.png" alt=""/></li>
		...
		<div class="right-top color_333 bgcolf5">
			...
			<a href="${action.getUrl("d", "list")}" class="newcarlist_letterlist"></a>
			<a href="${action.getUrl("d", "view")}" class="newcarlist_picturelist selected"></a>
		...
		<div class="center" id="carRight">
			<ul class="newcarlist_viewul clearfix">
			...
			<li>
				<a class="newcarlist_view_detail_carpic" href="<@baseUrl.home />/no-haggle/${e.vehicleId}" target="_blank" onClick="_traceEvent('车辆图片', '点击', '${(e.vehicleId)!},${(e.vehicle.brand)!} ${(e.vehicle.vehicleModel)!}<#if e.vehicle.sweptVolumeStandard?? && e.vehicle.sweptVolumeStandard?starts_with("欧")>(进口)</#if> ${(e.vehicle.selledName)!}','5','false');">
					<img original="<#if e.vehicle.firstPhotoUrl??><@baseUrl.image />/${(action.getThumbImgUrl("240X180", "${(e.vehicle.firstPhotoUrl)!}"))!}<#else><@baseUrl.static />/common/build/1.00/images/noimg.png</#if>" height="180" width="240" alt="" class="newcarlist_carPic" />
				</a>
				<div class="newcarlist_view_detail">
					<div class="newcarlist_view_detail_top">
						<#if e.vehicle.certified?? && e.vehicle.certified>
							<img src="<@baseUrl.static />/used_car/build/2.00/images/list/e_logo.png" alt="" class="e-logo used_listview" />
						</#if>
						...
						<a target="_blank" class="" href="<@baseUrl.home />/no-haggle/${e.vehicleId}" 
							...
							onClick="_traceEvent('车辆标题', '点击', '${e.vehicleId},${(e.vehicle.brand)!} ${(e.vehicle.vehicleModel)!}<#if e.vehicle.sweptVolumeStandard?? && e.vehicle.sweptVolumeStandard?starts_with("欧")>(进口)</#if>${(e.vehicle.selledName)!}','7','false');">
							${(e.vehicle.brand)!} ${(e.vehicle.vehicleModel)!}<#if e.vehicle.sweptVolumeStandard?? && e.vehicle.sweptVolumeStandard?starts_with("欧")>(进口)</#if>${(tempSelledName)!}
						</a>
					</div>
					<div class="newcarlist_view_detail_wrap">
						<div class="newcarlist_view_detail_price">
							<#if e.hidden?? && e.hidden><strong>面议</strong><#else><strong>${((e.price / 10000)?string("0.00"))!"0.00"}</strong><span>万元</span></#if>
						</div>
						<#if e.vehicle.newVehicleSuggestionPrice?? && (e.vehicle.newVehicleSuggestionPrice > 0)>
							<#if e.taxBuy??>
								<#if e.price??>
									<div class="newcarlist_view_detail_suggest">为您节省${(((e.vehicle.newVehicleSuggestionPrice * 10000 + e.taxBuy - e.price)/10000)?string("0.00"))!"0.00"}万元</div>
								<#else>
									<div class="newcarlist_view_detail_suggest">为您节省 ${(((e.vehicle.newVehicleSuggestionPrice * 10000 + e.taxBuy) / 10000)?string("0.00"))!"0.00"}万元</div>
								</#if>
							<#else>
								<#if e.price??>
									<div class="newcarlist_view_detail_suggest">为您节省 ${(((e.vehicle.newVehicleSuggestionPrice * 10000 - e.price)/10000)?string("0.00"))!"0.00"}万元</div>
								<#else>
									<div class="newcarlist_view_detail_suggest">为您节省 ${((e.vehicle.newVehicleSuggestionPrice)?string("0.00"))!"0.00"}万元</div>
								</#if>
							</#if>
						<#else>
							<div class="newcarlist_view_detail_suggest">&nbsp;</div>
						</#if>
					</div>
					<div class="newcarlist_view_detail_context">
						<span class="brdrig">${(e.city)!}</span><span class="brdrig">${(((e.vehicle.displayMileage)/10000)?string("0.00"))!"0.00"}万公里</span><span>${(e.vehicle.registerLicenseYears?substring(0,4))!}年${(e.vehicle.registerLicenseYears?substring(4))!}月上牌</span>
					</div>
					<#if e.vehicle.vehicleSrcType?? && (e.vehicle.vehicleSrcType == 1)>
						<div class="newcarlist_view_detail_shop">
							<img src="<@baseUrl.static />/used_car/build/2.00/images/renzheng.png" alt="该商家已通过汽车街平台认证" title="该商家已通过汽车街平台认证" />
							<a href="<@baseUrl.newHome />/dealerShop/index2?orgSid=${(e.vehicle.vehicleSrcId)!}"><span>${(action.getUsedcarDealerName(e.vehicle.vehicleSrcId))!}</span></a>
						</div>
					<#else>
						<div class="newcarlist_view_detail_shop">其他商家</div>
					</#if>
				</div>
			</li>
			</#list>
