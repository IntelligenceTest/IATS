...
<div class="mainblock">
    ...
    <div class="container">
   		<div class="bi_wrap">
   			<div class="container_top">
				<form:form id="searchForm" commandName="auctionDataUploadVO" modelAttribute="auctionDataUploadVO" action="${ctx}/adumanage/index">
                ...
                <form style="display: none;" id="exportForm" action="${ctx}/adumanage/exportExcel" method="post">
	                 <c:choose>
        ...
        <shiro:hasPermission name="auct:adumanage:importFile">
           <input id="import" class="addbtn query_btn_2" type="submit" value="导入" onclick="javacript: openImportDiv();" />
        </shiro:hasPermission>
        <shiro:hasPermission name="auct:adumanage:exportFile">
        ...
        <shiro:hasPermission name="auct:adumanage:deleteFile">
	...
	<div class="UED_hide" id="importDiv">
        <div class="pop_box" style="width: 420px;">
            ...
            <div class="s_box">
                <form id="uploadForm" class="searchForm" action="${ctx}/adumanage/uploadFile" method="post" enctype="multipart/form-data">
...
$(function(){
    ...
    kkpager.generPageHtml({
        ...
        //链接前部
        hrefFormer: "${ctx}/adumanage/index",
        //链接算法
        getLink: function(n){
...
function openExportDiv() {
	if (confirm("是否确认导出？")) {
		var queryString = $('#searchForm').serialize();
	    var actionUrl = "${ctx}/adumanage/exportExcel?" + queryString;
...
function deleteData() {
	...
	if (confirm("是否确认删除？")) {
		 $.get("${ctx}/adumanage/deleteFile?auctionId=" + auctionId, function(result, status) {
            if (result.success) {
            	alert(result.msg);
            	location.href="${ctx}/adumanage/index";
            } else {
            	alert(result.msg);
            	return;
            };
...
function uploadData() {
	...
	if (confirm("是否确认导入？")) {
		 $.get("${ctx}/adumanage/uploadFile?fileId=" + fileId+"&auctionId="+auctionId, function(result, status) {
            if (result.success) {
            	alert(result.msg);
            	location.href="${ctx}/adumanage/index";
            }else {
...
function ajaxFileUpload() {
    ...
    $.ajaxFileUpload({
        url: '${ctx}/adumanage/uploadCheck', //用于文件上传的服务器端请求地址
