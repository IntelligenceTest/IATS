...
<div id="apply">
	<#if prepayOrder.item.status == 0>
		<blockquote class="notice">
        	...
        	<a href="<@baseUrl.home />/no-haggle/netpay?orderId=${prepayOrder.id}" class="pay_for">立即支付</a>
    	</blockquote>
    ...
    <#elseif prepayOrder.item.status == 4 || prepayOrder.item.status == 5>
    	<blockquote class="notice">
			<img src="<@baseUrl.static />/user/build/1.00/images/success.png" width="66" alt=""  class="notice_icon">
        	<h3><#if prepayOrder.item.status == 4>订金支付成功!<#else>全款支付成功!</#if></h3>
        	<#if prepayOrder.item.status == 4><p>请在3天内到店签订购车合同并支付余款，我们的工作人员也会尽快联系您。</p></#if>
        	<div class="other">
	...
	<#elseif prepayOrder.item.status == 8>
		<blockquote class="notice">
			<img src="<@baseUrl.static />/user/build/1.00/images/success.png" width="66" alt=""  class="notice_icon">
        	<h3>交易完成，祝您用车愉快!</h3>
        	<div class="other">
    ...
    <div class="apply_exchange">
        <div class="your_car">
            ...
            <div class="order_price">
                <p>应付订金： <span>￥${(despositAmt?string("0.00"))!"0.00"}</span></p><p>车辆总价：￥${(prepayOrder.item.finalPrice)!}</p>
            </div>
            <div class="clear_fix"></div>
        </div>
    </div>
