...
<div class="hscroll_600 mb_20" id="facade_div">
    <table id="upholstery_table" class="form_table mb_20 width_percent_100">
        <tbody>
            <tr>
                <td rowspan="${interiorCount}" class="detection">
                    <div id="upholstery" class="fixed_facade">
                        <p>内饰检测：共包含${interiorCount}个监测点，已用圆圈标出</p>
                        <div class="carimg-3">
                            <c:forEach var="checkitem" items="${vehicle.checkitemList}">
                                <c:set var="i" value="${checkitem.itemOrder}" />
                                <c:if test="${checkitem.category==2&&checkitem.subCategory==5&&checkitem.itemGroup==1}">
                                    <c:choose>
                                        <c:when test="${checkitem.good}">
                                            <span id="interiorList${i}CarImg" style="top:${checkitem.axisy}px; left:${checkitem.axisx}px;">${i}</span>
                                        </c:when>
                                        <c:otherwise>
                                            <spring:eval expression="@auctionDictionaryService.choiceImgUrl('350X200', checkitem.photoUrl)" var="damagePhotoSmall" />
                                            <span id="interiorList${i}CarImg" class="e" style="cursor: pointer; top:${checkitem.axisy}px; left:${checkitem.axisx}px;">${i}
